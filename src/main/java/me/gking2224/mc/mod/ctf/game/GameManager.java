...
public class GameManager {
  ...
  private Bounds getNewGameBounds(GameOptions options) {
    ...
    while (!suitable) {
      int xBound = 100000;// server.getMaxWorldSize() - (gameChunksX*16);
      int startX = world.rand.nextInt(xBound) * 16;
      int zBound = 100000;// server.getMaxWorldSize() - (gameChunksZ*16);
      ...
      GameWorldManager gwm = GameWorldManager.get();
      suitable = !boundaryClashes(bounds) && gwm.isSuitableForGame(bounds);
  ...
  private boolean boundariesOverlap(Bounds bounds1, Bounds bounds2) {
    BlockPos f1 = bounds1.getFrom();
    BlockPos to1 = bounds1.getTo();
    BlockPos f2 = bounds2.getFrom();
    BlockPos to2 = bounds2.getTo();

    boolean fullyAbove = f2.getZ() > to1.getZ() && to2.getZ() > to1.getZ();
    boolean fullyBelow = f2.getZ() < f1.getZ() && to2.getZ() < f1.getZ();

    boolean fullyRight = f2.getX() > to1.getX() && to2.getX() > to1.getX();
    boolean fullyLeft = f2.getX() < f1.getX() && to2.getX() < f1.getX();
  ...
  public Optional<Game> getPlayerActiveGame(String name) {
    return Optional.ofNullable(games.values().stream()
  ...
  public void broadcastToAllPlayers(Game game, String msg) {
    game.getAllPlayers().forEach(
            (player) -> broadCastMessageToPlayer(player, toIText(msg)));
  }
  ...
  {
    broadcastToAllPlayers(game, format("Player %s won the round for team %s!",
  ...
  private void resetGame(Game game) {
    server.addScheduledTask(() -> MinecraftForge.TERRAIN_GEN_BUS
  ...
  public Set<Game> getAllGamesWithPlayer(String playerName) {
    return games.values().stream().filter((g) -> g.containsPlayer(playerName))
